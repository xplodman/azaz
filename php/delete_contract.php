...
$remove_contract = mysqli_query($con, "UPDATE `owner_has_property` SET `status` = '0', `update_time` = CURDATE() WHERE `owner_has_property`.`id` = '$contract_id';")or die(mysqli_error($con));

$remove_payments = mysqli_query($con, "UPDATE `payment` SET `update_time` = NOW(), `removed` = '1' WHERE `payment`.`owner_id` = '$owner_info[id]';")or die(mysqli_error($con));
